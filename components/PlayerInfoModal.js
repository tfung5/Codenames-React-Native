...
export default ({ visible, setVisible, playerInfo }) => {
  const listRedPlayers = () => {
    ...
    for (const key of items) {
      if (key.team === "RED") {
        redNames.push(
          <Text style={styles.chartText} key={key.id}>
            {key.name}
          </Text>
        );
    ...
    return (
      <View style={styles.chartRow}>
        <View style={styles.chartColumn}>{redNames}</View>
        <View style={styles.chartColumn}>{redRoles}</View>
      </View>
    );
  };
  ...
  const listBluePlayers = () => {
    ...
    return (
      <View View style={styles.chartRow}>
        <View style={styles.chartColumn}>{blueNames}</View>
        <View style={styles.chartColumn}>{blueRoles}</View>
      </View>
    );
  };
